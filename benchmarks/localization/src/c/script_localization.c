...
int main(int argc, char* argv[])
{
 ...
 
    {
        ...
        eul1 = eul2quat(eulAngle);  //apx_possiblity 
        fFreeHandle(eulAngle);

        eulAngle = fSetArray(1, 3, 0);
        subsref(eulAngle, 0, 0) = pi;
        eul2 = eul2quat(eulAngle); //apx_possiblei
        
        fFreeHandle(randn);
        fFreeHandle(eulAngle);
    }
    
    quat = quatMul(eul1, eul2); //apx_possiblity
    fFreeHandle(eul1);
    fFreeHandle(eul2);
    ...
    while(1)
    {
        ...
        //Motion model
            
            {
                ...
                for(i=0; i<(n*3); i++)
                {
                    ...
                    asubsref(gyro, i) += asubsref(t1,i); //apx_possiblity
                ...
                fFreeHandle(abc);
                abc = fSetArray(1,3,1);
                t1 = fMtimes_mod(norm_gyro, abc); //apx_possiblity_here
                t = ffDivide(gyro, t1);
                fFreeHandle(t1);

                abcd = fMtimes_mod(sinA, abc); //apx_possiblity_here
                t1 = fTimes_mod(t, abcd); //apx_possiblity_here
     ...
     if( asubsref(sType,0) ==4)
     {
        ...
        for(i=0; i<(pos->height*pos->width); i++)
            tempSum = tempSum + asubsref(pos,i); //AdditionOp
        ...
        if(tempSum == 0)
        {
            ...
            for(i=0; i<(pos->height*pos->width); i++)
                asubsref(pos,i) = asubsref(t,i) + asubsref(t1,i); //AdditionOp
        ...
        else 
        {
            ...
            w = mcl(pos, Opos , temp_STDDEV_GPSPos);  //apx_possiblity
        ...
        if (OvelNorm>0.5)
        {
            ...
            
            {
                t = fSetArray(1,3,0);
                subsref(t,0,0) = 1;
                orgWorld = quatRot(t, qConj); //apx_possiblity
                fFreeHandle(t);
                fFreeHandle(qConj);
                t = fSetArray(3,3,0);
                asubsref(t,0) = 1;
    ...
    if( asubsref(sType,0) ==1)
    {
        ...
        w = mcl (t, Ovel, t1); //apx_possiblity
        generateSample(w, quat, vel, pos);
    ...
    if( asubsref(sType,0) ==3)
    {
        ...
        gtemp = fMtimes( ones, t);  //apx_possiblity
        gravity = quatRot(gtemp, quat); //apx_possiblity
...

        {
            ...
            int i;
            is = quatConj(quat);
            s = quatRot(vel, is); //apx_possiblity
            fFreeHandle(is);
            ...
            is = quatConj(quat);
            s = quatRot(accl, is); //apx_possiblity
            t = fDeepCopy(s);
            for(i=0; i<(s->height*s->width); i++)
            {
                asubsref(t,i) = 1/2*asubsref(s,i)*acclTimeInterval*acclTimeInterval; //apx_possiblity
            }
            ...
            for(i=0; i<(s->height*s->width); i++)
            {
                asubsref(s,i) = asubsref(s,i) * M_STDDEV_POS; //MultiplicationOp
            }
...
//            vel=vel+accl*acclTimeInterval+randn(n,3)*M_STDDEV_VEL; %??
            ...
            for(i=0; i<(accl->height*accl->width); i++)
                asubsref(t,i) = asubsref(accl,i) * acclTimeInterval; //MultiplicationOp
            ...
            for(i=0; i<(s->height*s->width); i++)
            {
                asubsref(s,i) = asubsref(s,i) * M_STDDEV_VEL; //MultiplicationOp
            }
