 ...
 **/
void print_stuff(I2D* i_) {
    //possibly here 
    for (int i = 0; i < i_->height; i++) {
        for (int j = 0; j < i_->width; j++) {
            printf("%d ", subsref( i_, i, j ));
        }
        printf("\n"); 
    }  
}
...
void print_stuff(F2D* i_) {
    //possibly here 
    for (int i = 0; i < i_->height; i++) {
        for (int j = 0; j < i_->width; j++) {
            printf("%f ", subsref( i_, i, j ));
        }
        printf("\n"); 
    }  
}
...
F2D* sift(F2D* I)
{
    ...
    F2D *descriptors;
    F2D **gss, *tfr, *frames_temp, *tfr_temp;
    F2D **dogss;
    I2D* i_, *j_, *s_, *octave_, *i__;
    F2D *oframes, *frames;
    int i, j, k, m, startRow, startCol, endRow, endCol, firstIn=1;
    float minVal;
    I2D* tx1, *ty1, *ts1;
    I2D* x1, *y1, *s1, *txys, *octave1;
    ...
    **/
    ...
    for(o=0; o<Octaves; o++)
    {
        ...
        **/
        ...
        octave_ = iSetArray(idx->height,idx->width,0); //behzad added
        {
            int i, j;
            for(i=0; i<idx->height; i++)
             {
                 for(j=0; j<idx->width; j++)
                 {
                    ...
                    subsref(octave_,i,j) = o; //behzad added
                 }
             }
        }

        {
            ...
            octave1 = iReshape(octave_, 1, (octave_->height*octave_->width)); //behzad added
            
            txys = iVertcat(y1, x1);
            i__ = iVertcat(txys, s1);
            i_ = iVertcat(i__, octave1); //behzad added
        }
       ...
       //behzad added
        ...
        if( firstIn == 0)
        {
            tfr_temp = fDeepCopy(frames_temp);
            fFreeHandle(frames_temp);
            frames_temp = fHorzcat(tfr_temp, oframes);
            fFreeHandle(tfr_temp);
        }
        else
            frames_temp = fDeepCopy(oframes);
       
        
        if (o == 4) {
            printf("wtf\n"); 
            print_stuff(frames_temp);
        }
        } 
        */ //end of behzad added
