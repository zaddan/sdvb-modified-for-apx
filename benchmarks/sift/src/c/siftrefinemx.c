  ...
  ** -------------------------------------------------------------- */ 
  ...
  {    
    ...
    for(p = 0 ; p < K ; ++p) 
    {
        ...
        toctave = asubsref(oframes, pptcount++);
...
#define Aat(i,j) (A[(i)+(j)*3])    
				...
				
        {
             ...
             sn <= S-1) 
		  {
            ...
            asubsref(buffer,buffCounter++) =  asubsref(oframes, pptcount-1);
          }
        }
      }
    }      
    ...
    {
      int i, j, k=0;
      int NL = buffCounter/4;
      //out = fMallocHandle(3, NL);
      out = fMallocHandle(4, NL);
        for(i=0; i<NL; i++)
        {
            for(j=0; j<4; j++)
            {
